...
if exists(select name from tempdb.dbo.sysobjects where name = 'bcchqin_ort') 
	drop table tempdb.dbo.bcchqin_ort

go 

select * into tempdb.dbo.bcotherincome from nebula.bcnp.dbo.bcotherincome
	where docdate between '01/01/2015' and '31/12/2015'
		and (docno like '%ORT%' or docno like '%ORV%')
...
select * into tempdb.dbo.bctranssub_otherincome from nebula.bcnp.dbo.bctranssub 
	where docno in (select docno from tempdb.dbo.bcotherincome)
select * into tempdb.dbo.bcchqin_ort from nebula.bcnp.dbo.bcchqin 
	where docno in (select docno from tempdb.dbo.bcotherincome)
go 
...
alter table tempdb.dbo.bcchqin_ort drop column roworder
...
delete bcchqin where docno in (select docno from tempdb.dbo.bcotherincome)
...
insert into bcchqin  select * from tempdb.dbo.bcchqin_ort
